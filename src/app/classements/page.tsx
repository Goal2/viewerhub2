...
// ---- MOCKS (remplace plus tard par ton API) ----
const MOCK_LB: Item[] = [
...
export default function Page() {
  ...
  const W = 560;
  const H = 140;
  const path = buildLinePath(MOCK_DAILY.map((d) => d.messages), W, H);
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-black text-white">
      ...
      <div className="relative z-10 grid h-full grid-rows-[auto,1fr]">
        <header className="flex items-center gap-4 px-6 py-4">
          ...
          <nav className="ml-1 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-1">
            <span className="rounded-xl bg-white/15 px-3 py-1.5 text-sm font-medium">Classements</span>
          </nav>
      ...
      <style jsx>{`
        ...
        .halo-float { animation: floatA 28s ease-in-out infinite; }
        .halo-float-slow { animation: floatB 36s ease-in-out infinite; }
      `}</style>
    </div>
  );
}
